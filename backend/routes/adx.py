...
from services.adx_service import CUTOFF_MONTHS as ADX_MONTHS, SCHEMA_VERSION as ADX_SCHEMA_VERSION, compute_adx_rows
from services.technical_utils import normalize_timeframe


bp = Blueprint('adx', __name__)

_cache = TTLCache(ttl_seconds=int(os.getenv('ADX_CACHE_TTL') or '1800'))
...
def _snapshot_path(timeframe: str) -> str:
  ...
  if timeframe == 'daily':
    return os.path.join(_snapshot_dir, 'snapshot_adx.json')
  return os.path.join(_snapshot_dir, f'snapshot_adx_{timeframe}.json')


def _build_payload(timeframe: str) -> Dict[str, Any]:
  rows, meta = compute_adx_rows(timeframe=timeframe)
...
def _is_valid(payload: Dict[str, Any] | None, timeframe: str) -> bool:
  ...
  return base_months == ADX_MONTHS and payload_tf == timeframe and meta.get('schemaVersion') == ADX_SCHEMA_VERSION
