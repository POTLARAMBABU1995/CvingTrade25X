...
STAGING_TABLE = 'NSE_NIFTY_REALTY_REAL_ESTATE_STAGING'
REFERENCE_TABLE = 'NSE_NIFTY_AUTO_STAGING'
SECTOR_CODE = 'REALTY_REAL_ESTATE'
SECTOR_NAME = 'Realty Real Estate'
INDEX_CODE = 'NIFTY_REALTY_REAL_ESTATE'
DISPLAY_ORDER = 50
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description='Load Realty Real Estate sector CSV symbols into Oracle staging table and sector master.'
    )
...
def _ensure_tables(cursor: Any) -> None:
    ...
    if not _index_exists(cursor, 'UK_NIFTY_REALTY_RE_SYM'):
        cursor.execute(
            f'CREATE UNIQUE INDEX UK_NIFTY_REALTY_RE_SYM ON {STAGING_TABLE} (SYMBOL)'
        )
