...
STAGING_TABLE = 'NSE_NIFTY_ELECTRONICS_SERVICES_CONSUMER_DURABLES_STAGING'
REFERENCE_TABLE = 'NSE_NIFTY_AUTO_STAGING'
SECTOR_CODE = 'ELEC_SERVICES_CONS_DURABLES'
SECTOR_NAME = 'Electronics & Services Consumer Durables'
INDEX_CODE = 'NIFTY_ELECTRONICS_SERVICES_CONSUMER_DURABLES'
DISPLAY_ORDER = 50
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description='Load Electronics & Services Consumer Durables sector CSV symbols into Oracle staging table and sector master.'
    )
...
def _ensure_tables(cursor: Any) -> None:
    ...
    if not _index_exists(cursor, 'UK_NIFTY_ELEC_SERV_CONS_DUR'):
        cursor.execute(
            f'CREATE UNIQUE INDEX UK_NIFTY_ELEC_SERV_CONS_DUR ON {STAGING_TABLE} (SYMBOL)'
        )
