...
STAGING_TABLE = 'NSE_NIFTY_ECOMMERCE_ERETAI_STAGING'
REFERENCE_TABLE = 'NSE_NIFTY_AUTO_STAGING'
SECTOR_CODE = 'ECOMMERCE_ERETAI'
SECTOR_NAME = 'E-Commerce E-Retail'
INDEX_CODE = 'NIFTY_ECOMMERCE_ERETAI'
DISPLAY_ORDER = 50
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description='Load E-Commerce E-Retai sector CSV symbols into Oracle staging table and sector master.'
    )
...
def _ensure_tables(cursor: Any) -> None:
    ...
    if not _index_exists(cursor, 'IDX_NSE_ECOMMERCE_ERETAI_SYM'):
        cursor.execute(
            f'CREATE UNIQUE INDEX IDX_NSE_ECOMMERCE_ERETAI_SYM ON {STAGING_TABLE} (SYMBOL)'
        )
