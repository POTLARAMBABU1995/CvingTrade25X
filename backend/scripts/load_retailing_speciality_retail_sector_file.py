...
STAGING_TABLE = 'NSE_NIFTY_RETAILING_SPECIALITY_RETAIL_STAGING'
REFERENCE_TABLE = 'NSE_NIFTY_AUTO_STAGING'
SECTOR_CODE = 'RETAILING_SPECIALITY_RETAIL'
SECTOR_NAME = 'Retailing Speciality Retail'
INDEX_CODE = 'NIFTY_RETAILING_SPECIALITY_RETAIL'
DISPLAY_ORDER = 50
...
SELECT table_name
FROM user_tables
WHERE table_name LIKE 'NSE_%_STAGING'
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description='Load Retailing Speciality Retail sector CSV symbols into Oracle staging table and sector master.'
    )
    ...
    parser.add_argument('--resolve-conflicts', action='store_true', help='Automatically delete symbols from old staging tables if they conflict.')
    return parser
...
def _ensure_tables(cursor: Any) -> None:
    ...
    if not _index_exists(cursor, 'IDX_NSE_RET_SPEC_RET_SYM'):
        cursor.execute(
            f'CREATE UNIQUE INDEX IDX_NSE_RET_SPEC_RET_SYM ON {STAGING_TABLE} (SYMBOL)'
        )
...
def _merge_staging_rows(cursor: Any, rows: list[dict[str, Any]]) -> None:
    ...
    for row in rows:
        cursor.execute(
            ...
            """,
            {'symbol': row['symbol'], 'sector_code': SECTOR_CODE}
        )
...
def run_load(csv_path: Path, *, dry_run: bool, resolve_conflicts: bool) -> dict[str, Any]:
...
def main(argv: list[str] | None = None) -> int:
    ...
    file_param = args.file or args.source_file
    if not file_param:
        print(json.dumps({'ok': False, 'message': 'Missing CSV file parameter. Use --file or --source-file.'}), file=sys.stderr)
        return 1

    csv_path = Path(file_param).expanduser().resolve()
    if not csv_path.exists() or not csv_path.is_file():
        print(json.dumps({'ok': False, 'message': f'CSV file not found: {csv_path}'}), file=sys.stderr)
        return 1

    try:
        result = run_load(csv_path, dry_run=bool(args.dry_run), resolve_conflicts=bool(args.resolve_conflicts))
    except Exception as exc:
