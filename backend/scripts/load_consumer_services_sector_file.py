...
STAGING_TABLE = 'NSE_NIFTY_CONSUMER_SERVICES_STAGING'
REFERENCE_TABLE = 'NSE_NIFTY_AUTO_STAGING'
SECTOR_CODE = 'CONSUMER_SERVICES'
SECTOR_NAME = 'Consumer Services'
INDEX_CODE = 'NIFTY_CONSUMER_SERVICES'
DISPLAY_ORDER = 50
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description='Load Consumer Services sector CSV symbols into Oracle staging table and sector master.'
    )
...
def _ensure_tables(cursor: Any) -> None:
    ...
    if not _index_exists(cursor, 'UK_NIFTY_CONS_SERV_SYM'):
        cursor.execute(
            f'CREATE UNIQUE INDEX UK_NIFTY_CONS_SERV_SYM ON {STAGING_TABLE} (SYMBOL)'
        )
