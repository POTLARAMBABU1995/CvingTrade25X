...
STAGING_TABLE = 'NSE_NIFTY_RUBBER_PRODUCTS_TYRES_STAGING'
REFERENCE_TABLE = 'NSE_NIFTY_AUTO_STAGING'
SECTOR_CODE = 'RUBBER_PRODUCTS_TYRES'
SECTOR_NAME = 'Rubber Products Tyres'
INDEX_CODE = 'NIFTY_RUBBER_PRODUCTS_TYRES'
DISPLAY_ORDER = 50
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description='Load Rubber Products Tyres sector CSV symbols into Oracle staging table and sector master.'
    )
...
def _ensure_tables(cursor: Any) -> None:
    ...
    if not _index_exists(cursor, 'UK_NIFTY_RUBBER_TYRES_SYM'):
        cursor.execute(
            f'CREATE UNIQUE INDEX UK_NIFTY_RUBBER_TYRES_SYM ON {STAGING_TABLE} (SYMBOL)'
        )
