...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description='Load Consumer Electronics sector CSV symbols into Oracle staging table and sector master.'
    )
...
def _ensure_tables(cursor: Any) -> None:
    ...
    if not _index_exists(cursor, 'UK_NIFTY_CONS_ELEC_SYM'):
        cursor.execute(
            f'CREATE UNIQUE INDEX UK_NIFTY_CONS_ELEC_SYM ON {STAGING_TABLE} (SYMBOL)'
        )
