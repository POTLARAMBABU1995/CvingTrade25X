...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description='Load Consumer Durables sector CSV symbols into Oracle staging table and sector master.'
    )
...
def _ensure_tables(cursor: Any) -> None:
    ...
    if not _index_exists(cursor, 'UK_NIFTY_CONS_DUR_SYM'):
        cursor.execute(
            f'CREATE UNIQUE INDEX UK_NIFTY_CONS_DUR_SYM ON {STAGING_TABLE} (SYMBOL)'
        )
...
def _merge_staging_rows(cursor: Any, rows: list[dict[str, Any]]) -> None:
    ...
    csv_symbols = [row['symbol'] for row in rows]
    if csv_symbols:
        binds = {f'sym_{i}': sym for i, sym in enumerate(csv_symbols)}
        bind_clause = ', '.join(f':sym_{i}' for i in range(len(csv_symbols)))
        cursor.execute(f"DELETE FROM {STAGING_TABLE} WHERE SYMBOL NOT IN ({bind_clause})", binds)
