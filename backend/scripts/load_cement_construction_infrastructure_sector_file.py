...
SECTORS = {
    'Cement & Cement Products': {
        'code': 'CEMENT_CEMENT_PRODUCTS',
        'table': 'NSE_NIFTY_CEMENT_CEMENT_PRODUCTS_STAGING',
        'index': 'NIFTY_CEMENT_CEMENT_PRODUCTS',
        'order': 56
    },
    'Construction': {
        'code': 'CONSTRUCTION',
        'table': 'NSE_NIFTY_CONSTRUCTION_STAGING',
        'index': 'NIFTY_CONSTRUCTION',
        'order': 57
    },
    'Construction Materials': {
        'code': 'CONSTRUCTION_MATERIALS',
        'table': 'NSE_NIFTY_CONSTRUCTION_MATERIALS_STAGING',
        'index': 'NIFTY_CONSTRUCTION_MATERIALS',
        'order': 58
    },
    'Infrastructure': {
        'code': 'INFRASTRUCTURE',
        'table': 'NSE_NIFTY_INFRASTRUCTURE_STAGING',
        'index': 'NIFTY_INFRASTRUCTURE',
        'order': 59
    }
}
...
WHERE utc.column_name = 'SYMBOL'
  ...
  AND utc.table_name NOT IN (:table1, :table2, :table3, :table4)
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description='Load Cement, Construction & Infrastructure sector CSV symbols into Oracle staging tables.'
    )
    parser.add_argument('--file', help='Path to CSV file.')
    parser.add_argument('--source-file', help='Alternative CLI argument for path to CSV file.')
    ...
    with csv_path.open('r', encoding='utf-8-sig', newline='') as handle:
        ...
        for row_number, raw in enumerate(reader, start=2):
            ...
            try:
                ...
                raw_sector = raw.get(sector_key, '')
                if not raw_sym:
                    raise ValueError(f'Row {row_number}: symbol is required.')
                if not raw_sector:
                    raise ValueError(f'Row {row_number}: sector is required.')

                sector_norm = _collapse_spaces(raw_sector)
                matched_sector_name = None
                for key in SECTORS.keys():
                    if key.lower().replace('&', 'and').replace(' ', '') == sector_norm.lower().replace('&', 'and').replace(' ', ''):
                        matched_sector_name = key
                        break
                
                if not matched_sector_name:
                    summary['failedRows'] += 1
                    errors.append(f"Row {row_number}: Sector '{raw_sector}' is not recognized")
                    continue

                norm = normalize_symbol(raw_sym)
                if norm == 'SYMBOL' or not norm:
                    continue
                ...
                rows.append({
                    'symbol': symbol,
                    'sectorName': matched_sector_name,
                    'sectorCode': SECTORS[matched_sector_name]['code'],
                    'stagingTable': SECTORS[matched_sector_name]['table'],
                })
...
def _ensure_master_rows(cursor: Any) -> None:
    for sector_name, conf in SECTORS.items():
        cursor.execute(
            """
...
def main(argv: list[str] | None = None) -> int:
    ...
    csv_file = args.file or args.source_file
    if not csv_file:
        print(json.dumps({'ok': False, 'message': 'Missing required argument: --file or --source-file'}), file=sys.stderr)
        return 1
