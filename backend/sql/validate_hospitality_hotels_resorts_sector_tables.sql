PROMPT Validating Hospitality Hotels & Resorts sector staging table
SET DEFINE OFF;

PROMPT [1] Table presence
SELECT table_name
FROM user_tables
WHERE table_name IN ('NSE_NIFTY_HOSPITALITY_HOTELS_RESORTS_STAGING')
ORDER BY table_name;

PROMPT [2] Master code
SELECT sector_code, sector_name, index_code, display_order
FROM nse_sector_master
WHERE sector_code = 'HOSPITALITY_HOTELS_RESORTS';

PROMPT [3] Staging count
SELECT 'NSE_NIFTY_HOSPITALITY_HOTELS_RESORTS_STAGING' AS table_name, COUNT(*) AS row_count
FROM NSE_NIFTY_HOSPITALITY_HOTELS_RESORTS_STAGING;
...
WHERE symbol IN (SELECT symbol FROM NSE_NIFTY_HOSPITALITY_HOTELS_RESORTS_STAGING)
  AND sector_code <> 'HOSPITALITY_HOTELS_RESORTS';

PROMPT [6] Join to DEV raw table
SELECT COUNT(DISTINCT r.symbol) AS joined_symbols_count
FROM NSE_NIFTY_HOSPITALITY_HOTELS_RESORTS_STAGING r
