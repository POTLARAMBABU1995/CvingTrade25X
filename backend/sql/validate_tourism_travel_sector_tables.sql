PROMPT Validating Tourism & Travel sector staging table
SET DEFINE OFF;

PROMPT [1] Table presence
SELECT table_name
FROM user_tables
WHERE table_name IN ('NSE_NIFTY_TOURISM_TRAVEL_STAGING')
ORDER BY table_name;

PROMPT [2] Master code
SELECT sector_code, sector_name, index_code, display_order
FROM nse_sector_master
WHERE sector_code = 'TOURISM_TRAVEL';

PROMPT [3] Staging count
SELECT 'NSE_NIFTY_TOURISM_TRAVEL_STAGING' AS table_name, COUNT(*) AS row_count
FROM NSE_NIFTY_TOURISM_TRAVEL_STAGING;
...
WHERE symbol IN (SELECT symbol FROM NSE_NIFTY_TOURISM_TRAVEL_STAGING)
  AND sector_code <> 'TOURISM_TRAVEL';

PROMPT [6] Join to DEV raw table
SELECT COUNT(DISTINCT r.symbol) AS joined_symbols_count
FROM NSE_NIFTY_TOURISM_TRAVEL_STAGING r
