PROMPT Validating Transport, Aviation & Logistics sector staging table
SET DEFINE OFF;
...
FROM user_tables
WHERE table_name IN (
    'NSE_NIFTY_TRANSPORT_AVIATION_LOGISTICS_STAGING'
)
ORDER BY table_name;
...
WHERE sector_code = 'TRANSPORT_AVIATION_LOGISTICS';

PROMPT [3] Staging count
SELECT 'NSE_NIFTY_TRANSPORT_AVIATION_LOGISTICS_STAGING' AS table_name, COUNT(*) AS row_count
FROM NSE_NIFTY_TRANSPORT_AVIATION_LOGISTICS_STAGING;
...
WITH other_sector_tables AS (
    ...
    WHERE utc.column_name = 'SYMBOL'
      ...
      AND utc.table_name <> 'NSE_NIFTY_TRANSPORT_AVIATION_LOGISTICS_STAGING'
),
cross_sector_hits AS (
    SELECT t.table_name, UPPER(TRIM(a.symbol)) AS symbol
    FROM NSE_NIFTY_TRANSPORT_AVIATION_LOGISTICS_STAGING a
