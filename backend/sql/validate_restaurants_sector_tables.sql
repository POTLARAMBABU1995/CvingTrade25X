PROMPT Validating Restaurants sector staging table
SET DEFINE OFF;

PROMPT [1] Table presence
SELECT table_name
FROM user_tables
WHERE table_name IN ('NSE_NIFTY_RESTAURANTS_STAGING')
ORDER BY table_name;

PROMPT [2] Master code
SELECT sector_code, sector_name, index_code, display_order
FROM nse_sector_master
WHERE sector_code = 'RESTAURANTS';

PROMPT [3] Staging count
SELECT 'NSE_NIFTY_RESTAURANTS_STAGING' AS table_name, COUNT(*) AS row_count
FROM NSE_NIFTY_RESTAURANTS_STAGING;
...
WHERE symbol IN (SELECT symbol FROM NSE_NIFTY_RESTAURANTS_STAGING)
  AND sector_code <> 'RESTAURANTS';
