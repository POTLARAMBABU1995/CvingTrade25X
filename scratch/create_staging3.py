...
def run_script():
    ...
    USING (
        SELECT 'AUTO_COMPONENTS_EQUIPMENTS' AS sector_code, 'Auto Components & Equipments' AS sector_name FROM dual
    ) src
    ON (tgt.SECTOR_CODE = src.sector_code)
    WHEN NOT MATCHED THEN INSERT (SECTOR_CODE, SECTOR_NAME) VALUES (src.sector_code, src.sector_name)
    WHEN MATCHED THEN UPDATE SET tgt.SECTOR_NAME = src.sector_name
    """
