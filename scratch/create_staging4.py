...
def run_script():
    ...
    USING (
        SELECT 'AUTO_COMPONENTS_EQUIPMENTS' AS sector_code, 'Auto Components & Equipments' AS sector_name, 'AUTO_COMPONENTS_EQUIPMENTS' AS index_code FROM dual
    ) src
    ON (tgt.SECTOR_CODE = src.sector_code)
    WHEN NOT MATCHED THEN INSERT (SECTOR_CODE, SECTOR_NAME, INDEX_CODE) VALUES (src.sector_code, src.sector_name, src.index_code)
