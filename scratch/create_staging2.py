...
def run_script():
    ...
    USING (
        SELECT 'AUTO_COMPONENTS_EQUIPMENTS' AS sector_code, 'Auto Components & Equipments' AS sector_name, 'NSE_NIFTY_AUTO_COMPONENTS_EQUIPMENTS_STAGING' AS staging_table FROM dual
    ) src
    ON (tgt.SECTOR_CODE = src.sector_code)
    WHEN NOT MATCHED THEN INSERT (SECTOR_CODE, SECTOR_NAME, STAGING_TABLE) VALUES (src.sector_code, src.sector_name, src.staging_table)
    WHEN MATCHED THEN UPDATE SET tgt.SECTOR_NAME = src.sector_name, tgt.STAGING_TABLE = src.staging_table
    """
