...
def run_script():
    ...
    table_sql = """
    CREATE TABLE NSE_NIFTY_AUTO_COMPONENTS_EQUIPMENTS_STAGING (
        SYMBOL VARCHAR2(50) NOT NULL,
        SECTOR_CODE VARCHAR2(100),
        TRADING_DATE DATE,
        OPEN NUMBER,
        HIGH NUMBER,
        LOW NUMBER,
        CLOSE NUMBER,
        VOLUME NUMBER,
        CONSTRAINT PK_NSE_AUTO_COMP_EQUIP_STG PRIMARY KEY (SYMBOL)
    )
    """
    
    try:
        cursor.execute("SELECT table_name FROM user_tables WHERE table_name = 'NSE_NIFTY_AUTO_COMPONENTS_EQUIPMENTS_STAGING'")
    ...
    try:
        cursor.execute("CREATE INDEX IDX_NSE_AUTO_COMP_EQUIP_STG_DATE ON NSE_NIFTY_AUTO_COMPONENTS_EQUIPMENTS_STAGING(TRADING_DATE)")
        print("Index created.")
    except Exception as e:
    ...
    # Also seed the staging table with empty records to trigger UI 
    seed_sql = """
